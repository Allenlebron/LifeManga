...
iOS 版同源的浏览器版本现已上线：[https://lifemanga-web.pages.dev/](https://lifemanga-web.pages.dev/)

把生活照转成漫画的核心功能 1:1 移植，加一个 iOS 端没有的本地 IndexedDB 持久化（关浏览器照样不丢）+ 加到主屏幕变 PWA + Cloudflare Worker 后端撑长任务。详细说明在 [`web/README.md`](./web/README.md) 和 [`worker/README.md`](./worker/README.md)。

技术栈：Vue 3 + Vite + Tailwind 4 (前端) + Cloudflare Worker + Durable Object (后端)。所有代码 MIT 开源。

### 自动部署到 Cloudflare（GitHub Actions）

仓库带了 `.github/workflows/deploy-web.yml` 和 `deploy-worker.yml`，**push 到 main 自动部署 web + worker**。要让它们工作，你需要在 GitHub repo 设置两个 secret：

1. **CLOUDFLARE_API_TOKEN**：到 [https://dash.cloudflare.com/profile/api-tokens](https://dash.cloudflare.com/profile/api-tokens) → Create Token → 用 "Edit Cloudflare Workers" 模板（自带 Pages + Workers 编辑权限）→ 复制 token
2. **CLOUDFLARE_ACCOUNT_ID**：在 Cloudflare dashboard 主页右栏能看到（一串 32 位 hex）

把这两个值贴进 GitHub repo → Settings → Secrets and variables → Actions → New repository secret，名字大小写要跟上面一致。

之后每次改 `web/**` 自动跑 web build + 部署到 Pages，每次改 `worker/**` 自动跑 tsc 类型检查 + 部署 worker。失败会在 Actions tab 看到红色 X。
